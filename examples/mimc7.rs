...
// 91 round constants C_i for i = 0 to 90
...
// function for generating an ir circuit, taking halo2 native fixed columns
fn mimc7_circuit<F: FieldExt>(
    ...
    // creates an ast circuit, taking a function input "def", which defines the forward signal, lookup, fixed column, step types
    let mimc7 = circuit::<F, (F, F), (F, F, F, F), _>("mimc7", |ctx| {
        ...
        // common pattern of functions manipulating context
        let x = ctx.forward("x");
        let k = ctx.forward("k");
        let c = ctx.forward("c");
        ...
        // populate the lookup columns to the FixedGenContext
    ...
    // create new compiler with cell manager and step selector
    let compiler = Compiler::new(SingleRowCellManager {}, SimpleStepSelectorBuilder {});
    
    // compiles ast circuit into ir circuit
